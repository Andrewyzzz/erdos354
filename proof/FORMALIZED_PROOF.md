...
above; if they intersect, gaps do not increase. Hence $`\mathrm{gap}(P_n) \le N`$ on $`[0,S_n]`$.
...
If $`\mathrm{span}(W)\ge m\ge1`$ and $`\mathrm{gap}(W)\le k`$, then
...
\begin{array}{ll}
dKp+u_1,&dKq+v_1,\\[0pt]
2dKp+2u_1+u_2,&2dKq+2v_1+v_2,\\[0pt]
...
```math
\mathrm{gap}(W_t)\le k,\qquad
\mathrm{span}(W_t)\ge b_t\ge D_t.
```
